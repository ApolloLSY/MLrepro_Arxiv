...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/lung_cacner/GSE114989_series_matrix_2.txt", "r"
) as file:
...
# read data
data_df = pd.read_table('/data/LSY/z_preparing_and_parts/lung_cacner/GSE114989_series_matrix.txt',
                           index_col=0, 
                           header=0,
                           delimiter='\t')
data_df = data_df.T

data_df = data_df[data_df.index.str.startswith('SAMPLE')]
...
# save data
data_df.to_csv(
    "/data/LSY/z_preparing_and_parts/lung_cacner/data_valid.txt",
    index=True,
    header=True,
    sep="\t",
)
