...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Asthma/GSE52074_series_matrix.txt", "r"
) as file:
...
# find status
metadata_df["status"] = metadata_df.iloc[:, 12].apply(lambda x: 0 if "non" in x else 1)
...
# save data
data_df.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_Asthma/data_valid.txt",
    index=True,
    header=True,
    sep="\t",
)
