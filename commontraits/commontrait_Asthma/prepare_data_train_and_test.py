...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Asthma/GSE85566_series_matrix.txt", "r"
) as file:
...
# find status
metadata_df["status"] = metadata_df.iloc[:, 11].apply(
    lambda x: 1 if "Asthma" in x else 0 if "Control" in x else None
)
...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Asthma/GSE104471_series_matrix.txt",
    "r",
) as file:
    lines = file.readlines()
...
# find status
metadata_df["status"] = metadata_df.iloc[:, 11].apply(
    lambda x: 1 if "Asthma" in x else 0 if "Normal" in x else None
)
metadata_df = metadata_df.set_index("Sample_geo_accession")
