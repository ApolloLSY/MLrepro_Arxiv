...
datafile = pd.read_table(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/data_valid.txt",
    index_col=0,
    header=0,
    delimiter="\t",
)
...
loaded_model = load("/data/LSY/z_preparing_and_parts/commontrait_ASD/model.joblib")
selected_features = load(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/model_selected_features.joblib"
)
...
valid_results.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/prediction_valid.txt",
    sep="\t",
    index=False,
    mode="a",
)
...
valid_scores.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/performance.txt",
    mode="a",
    index=True,
)
