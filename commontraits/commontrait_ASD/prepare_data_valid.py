...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/GSE108785_series_matrix.txt", "r"
) as file:
...
metadata_df["status"] = metadata_df.iloc[:, 10].apply(
    lambda x: 1 if "autism" in x else 0 if "control" in x else None
)
...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_ASD/GSE109042_series_matrix.txt", "r"
) as file:
    lines = file.readlines()
...
metadata_df["status"] = metadata_df.iloc[:, 12].apply(
    lambda x: 1 if "FASD" in x else 0 if "Control" in x else None
)
metadata_df = metadata_df.set_index("Sample_geo_accession")
