...
# read matrix
with open("/data/LSY/commontraits/Alzheimer/GSE66351_series_matrix.txt", "r") as file:
...
metadata_df["status"] = metadata_df["Sample_title"].apply(
    lambda x: 1 if "AD" in x else 0 if "CTRL" in x else None
)
...
# save data
data_df.to_csv(
    "/data/LSY/commontraits/Alzheimer/data_train.txt",
    index=True,
    header=True,
    sep="\t",
)
