...
# read matrix
with open(
    "/data/LSY/commontraits/Kabuki_syndrome/GSE116300_series_matrix_2.txt", "r"
) as file:
...
metadata_df["status"] = metadata_df.iloc[:, 9].apply(
    lambda x: 1 if "KS" in x else 0 if "normal" in x else None
)
...
data_df = pd.read_table(
    "/data/LSY/commontraits/Kabuki_syndrome/GSE116300_series_matrix.txt",
    index_col=0,
    header=0,
    delimiter="\t",
)
