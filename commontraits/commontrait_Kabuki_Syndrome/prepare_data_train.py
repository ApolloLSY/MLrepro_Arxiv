...
# read matrix
with open(
    "/data/LSY/commontraits/Kabuki_syndrome/GSE97362_series_matrix.txt", "r"
) as file:
...
# find status
metadata_df["status"] = metadata_df["Sample_title"].apply(
    lambda x: 1 if "KMT2D" in x else 0 if "Control" in x else None
)
...
# save data
data_df.to_csv(
    "/data/LSY/commontraits/Kabuki_syndrome/data_train.txt",
    index=True,
    header=True,
    sep="\t",
)
