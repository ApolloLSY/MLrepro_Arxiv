...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/GSE81961_series_matrix.txt", "r"
) as file:
...
metadata_df["status"] = metadata_df.iloc[:, 10].apply(
    lambda x: 1 if "Crohn" in x else 0 if "Control" in x else None
)
metadata_df = metadata_df.set_index("Sample_geo_accession")
...
# save data
data_df.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/data_test.txt",
    index=True,
    header=True,
    sep="\t",
)
