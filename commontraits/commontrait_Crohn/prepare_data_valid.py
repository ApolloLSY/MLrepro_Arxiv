...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/GSE32148_series_matrix.txt", "r"
) as file:
...
metadata_df["status"] = metadata_df.iloc[:, 12].apply(
    lambda x: 1 if "Crohn" in x else 0 if "normal" in x else None
)
...
# save data
data_df.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/data_valid.txt",
    index=True,
    header=True,
    sep="\t",
)
