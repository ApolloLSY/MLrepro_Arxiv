...
# read matrix
with open(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/GSE87648_series_matrix_2.txt",
    "r",
) as file:
...
metadata_df["status"] = metadata_df.iloc[:, 11].apply(
    lambda x: 1 if "CD" in x else 0 if "HL" in x else None
)
...
data_df = pd.read_table(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/GSE87648_series_matrix.txt",
    index_col=0,
    header=0,
    delimiter="\t",
)
...
# save data
data_df.to_csv(
    "/data/LSY/z_preparing_and_parts/commontrait_Crohn/data_train.txt",
    index=True,
    header=True,
    sep="\t",
)
